...
Production has its **own** ansible-vault password, stored as the
`ANSIBLE_VAULT_PASSWORD` secret on the matching GitHub Environment (and locally in
`ansible/.vault_pass`). A deploy decrypts everything with that key, so any secret
file the role touches must be encrypted with it.

Secret files in `files/` are named `<name>.prod.vault`, and the caller selects
the file (`r.officials_contacts_vault` / `r.twda_key_vault`, set in `deploy.yml`).
...
- The role copies `files/officials_contacts.json.prod.vault` (an `ansible-vault`
...
encrypt it with the prod vault password (`--vault-id` or the matching
`ANSIBLE_VAULT_PASSWORD_FILE`):

```bash
# from repo root, with prod's vault password.
cp <plaintext>/officials_contacts.json \
   ansible/roles/fastapi_backend/files/officials_contacts.json.prod.vault
ansible-vault encrypt ansible/roles/fastapi_backend/files/officials_contacts.json.prod.vault
git add ansible/roles/fastapi_backend/files/officials_contacts.json.prod.vault   # ciphertext is safe to commit
```
...
accident; only the `.prod.vault` file is allowed.

### Dev

The committed ciphertext is also the dev source — decrypt it directly (with
prod's vault password) into the gitignored runtime path the backend reads
...
- `FEEDBACK_GITHUB_PRIVATE_KEY` is the multi-line PEM — delivered out of band exactly like
  the TWDA key (next section): the role copies `files/feedback_github_app.pem.prod.vault`
  ...
  **derived from `env_dir`** in `vars.yml` (`{{ env_dir }}/feedback_github_app.pem`) so it
  can't drift from the copy destination — no vault scalar for the path.

Any of the three unset → `POST /api/feedback/` returns 503 (graceful degradation). Provide /
refresh the key the same way as the TWDA key, using `feedback_github_app.pem.prod.vault`.
...
- The role copies `files/twda_github_app.pem.prod.vault` (an `ansible-vault` file)
  ...
  (`{{ env_dir }}/twda_github_app.pem`) — no vault scalar for the path — so it can't drift
  from the copy destination (`twda.py:_load_private_key` reads the file when the value isn't inline PEM).
...
with the prod vault password:

```bash
# from repo root, with prod's vault password.
cp <downloaded>.pem \
   ansible/roles/fastapi_backend/files/twda_github_app.pem.prod.vault
ansible-vault encrypt ansible/roles/fastapi_backend/files/twda_github_app.pem.prod.vault
git add ansible/roles/fastapi_backend/files/twda_github_app.pem.prod.vault   # ciphertext is safe to commit
```

`files/.gitignore` blocks committing a plaintext `twda_github_app.pem` by accident;
only the `.prod.vault` file is allowed.
