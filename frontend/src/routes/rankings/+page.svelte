<script lang="ts">
  ...
  const categories: { value: RatingCategory; label: string }[] = [
...
<div class="max-w-4xl mx-auto px-4 py-6">
  ...
  {#if isSyncing && ratings.length === 0}
    <div class="text-center text-ash-400 py-8">
      ...
      <span class="ml-2">Loading rankings...</span>
    </div>
  {:else if filtered().length === 0}
    <div class="text-center text-ash-500 py-8">No rankings found</div>
  {:else}
            ...
            <tr class="border-b border-ash-800/50 hover:bg-ash-800/20">
              ...
              <td class="py-2 px-3">
                <a href="/users/{rating.user_uid}" class="text-ash-100 hover:text-crimson-400">
                  {userNames.get(rating.user_uid) ?? "Unknown"}
                </a>
              </td>
