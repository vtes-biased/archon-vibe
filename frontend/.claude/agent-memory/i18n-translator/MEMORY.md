...
- Files: `frontend/messages/{en,fr,es,pt,it}.json` — flat JSON key/value
- No i18n library import visible in files; keys used directly in Svelte components
- All 5 locales must be kept in sync; append new keys at end of each file before closing `}`

## File Structure Pattern
- Keys are grouped by feature prefix (e.g., `nav_`, `profile_`, `help_`, `tournament_`, etc.)
- No plural forms in JSON (not observed); interpolation uses `{variable}` syntax
- British English for `en.json` (though content appears to use standard spellings — verify per key)
...
- es: "Portal de desarrollador"
- pt: "Portal do desenvolvedor"
- it: "Portale sviluppatore"

## Help Section (added 2026-02-18)
