...
- **`sticky` carries its own inset**: `sticky top-*`/`bottom-*` resolve against the **scrollport**, not the shell's padding box, so a sticky header parks under the status bar and a sticky footer parks behind the bottom nav unless it adds the inset itself (`top-[calc(1rem+var(--spacing-safe-t))]`, `bottom-[calc(1rem+var(--spacing-navbar))]`). When the sticky element lives inside a scrolling panel rather than the page, the panel is the scrollport and there is nothing to clear — pass that in rather than assuming (see `TournamentDetailsForm`'s `inSheet`).
- **Modal heights**: `dvh`, never `vh` (on iOS `vh` is the *large* viewport, so a `90vh` sheet outgrows the visible area while the toolbar shows). Vertically centred panels cap at `85dvh`: that puts the top edge at `7.5dvh`, which is the smallest margin that still clears `safe-area-inset-top` on a Dynamic Island phone — `90dvh` clips the panel header under the status bar. Bottom sheets are anchored, so they cap wherever their content wants.
- **Full-height inside the shell**: use `.min-h-shell`, not `min-h-screen` — the shell's content box is `dvh` minus the top inset and the nav, so a `100vh` child overflows it and centres its content low.
