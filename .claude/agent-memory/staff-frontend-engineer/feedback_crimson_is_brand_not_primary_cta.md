...
**`app.css` mechanics:** crimson-* and ash-* ARE scale-inverted (adapt to light by rule, no new class). Amethyst is a stock-token hue with NO inversion → needs a new `.btn-danger` with an explicit `html.light` override (`#5B21B6`), hand-rolled like the old `btn-red`. Delete `brand`/`warning` variant keys from `<Button>`.
