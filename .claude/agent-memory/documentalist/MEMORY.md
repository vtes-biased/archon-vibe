...
- Synced object types: User, Sanction, Tournament, DeckObject, League, Promo (via SSE). VtesCard is static data. DeckObject is standalone (not embedded in Tournament).
...
- **A new synced type also earns its own ARCHITECTURE.md subsection** (`## <Type> System`/`Catalog`, mirroring "League System"/DeckObject-in-Card/Deck-System), not just a checklist tick in SYNC.md — that's where the field list, projection asymmetry, and any `entitled_level` exception belong. SYNC.md only gets the table-row-level mechanics (Access-Level Projections, IndexedDB indexes, `SPECS`). Added for Promo, 2026-07-17.
- **A gate/permission change on an existing tournament action (even a plain-REST one, not an engine event) tends to be restated in 3+ places within TOURNAMENTS.md alone** (state-machine States table's per-state action list, the REST endpoint table, the Permission Model table) **plus PRODUCT.md's terse Tournament-core feature bullet** — grep the action name across both files rather than trusting one hit. Example: Delete's gate changed from "Planned only" to "any state until pushed to VEKN" — all four mentions had to move together.
- **SYNC.md's "Adding a New Object Type" checklist steps for `_STREAM_TYPES`(`main.py`)/`OBJECT_TYPES`(`snapshots.py`) were stale** — both became `list(ObjectType)` auto-derivation at some point after the checklist was written (confirmed via `git log -p`), so a new `ObjectType` member needs no manual registration there. Fixed in the checklist 2026-07-17 (steps merged into the broadcast step, renumbered 1-8). If this checklist looks stale again, re-check `main.py`/`snapshots.py` before assuming a step was skipped.
