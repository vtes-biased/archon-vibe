...
- [Tournament-action route test reachability](trap_tournament_action_route_untested.md) — no route-level tournament_action test exists (post-engine hooks unreachable via the engine-contract pattern); also: event `vekn_id` is a gate, never stored — nothing to assert.
...
- [TWDA participation floor + twda_status test infra](trap_twda_participation_floor.md) — `maybe_submit_twda` gates on TWDA_MIN_PLAYERS distinct SEATED players (roundless fixtures no-op); patch `src.twda.is_configured`/`submit_twda_pr` + mock `_record_twda_status`.
