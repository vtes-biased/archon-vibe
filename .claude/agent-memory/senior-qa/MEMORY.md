...
- [TWDA participation floor + twda_status test infra](trap_twda_participation_floor.md) — `_maybe_submit_twda` gates on TWDA_MIN_PLAYERS distinct SEATED players (roundless fixtures no-op); patch `src.twda.is_configured`/`submit_twda_pr` + mock `_record_twda_status`.
