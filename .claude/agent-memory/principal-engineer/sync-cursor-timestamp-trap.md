...
**Why:** discovered reviewing pst #6 (frontend sync high-water-mark cursor), which advanced the persisted `since` cursor from `item.modified` (JSONB) while every server consumer filters on `modified_at` (column).
