...
description: Measured prod-dump facts for the vekn-ID-matching merge redesign — the vekn-less drop is NOT ref-free
metadata:
  type: project
---

Measured against the prod `archondb.dump.gz` (repo root, 19,003 members / 8,371 tournaments) on 2026-06-14 for the vekn-ID-matching merge redesign (vekn-ID matching replacing uid matching in `migrate_from_archon.py:merge_member`).
...
- 0 player_uids reference a non-existent member row (every player resolves to a members or member_deletions row today → ETL-first with uid-preservation has 0 orphans, which is why the rehearsal passed clean).
- 24 distinct sanction-issuer (judge) uids, all are member rows → `issued_by_uid` is a member-uid ref the remap MUST cover (build_sanction sets it at migrate_from_archon.py:622).

**Why:** the redesign's premise ("vekn-less members hold no tournament/deck refs because the engine requires a vekn_id for participants") is false for HISTORICAL data: old archon's `Register` event (`/Users/lpanhaleux/Developer/archon` events.py:64, `vekn: str = ""`) never enforced vekn_id; engine.py:136 stores the player unconditionally. The new engine's VeknIdRequired (engine/src/tournament/mod.rs:271) gates only the in-app path, which imported history bypasses.

**How it shipped:** vekn-less members are seeded as soft-deleted shells
...
remaining prod runs: if a future change ever reconsiders dropping
