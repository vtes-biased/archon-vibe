...
- Stop the bottom nav's two middle destinations reading as each other: Leagues wears a bar chart while Rankings wears a medal, and below `sm` there are no labels to correct the guess. **Done when** each of the six destinations carries an icon a reader maps to it unaided, the set is checked against all five locales' `aria-label`s, and `wiki/design.md` records the icon set under Navigation.
