...
- Ask a player to confirm before their own Drop Out goes through, with the same dialog the organizer's drop uses, reworded for the player and telling them they can check back in to undo it. Done when tapping Drop Out on the player's tournament view opens that confirmation and cancelling leaves them in the event, and the drop-out paragraph in [tournaments](wiki/tournaments.md) and the confirm-step rule in [design](wiki/design.md) record the self-drop as confirmed and why.
