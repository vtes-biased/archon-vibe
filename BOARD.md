...
- Submit the winner's deck from a fork we own, so the archive never has to trust us with write access to it: the auto-PR asks the archive's installation for permission to commit, is refused, and no winning deck has reached the TWDA since the feature went live. Push the branch and the deck file to our own fork, and use the archive's installation for nothing but opening the pull request, each installation asked for only the access it needs. **Done when** a finished event opens its pull request on the archive while holding no write access to it, and `wiki/dev.md` states the two installations and what each is granted. Context in `board/twda-fork-submission.md`.
