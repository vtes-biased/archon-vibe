...
- Restructure the community page around pin placement and give links their own country — a nationally pinned link appears in its country's card and a globally pinned one in the Global card whatever its platform; the page shows Global, your country (pinned channels, then social groups, then officials) and a country search instead of the full country list; content becomes one pool filtered by a language multiselect defaulting to the reader's language plus a media-kind facet; a link carries a country of its own defaulting to the owner's, with moderation and national pins keyed off it; Spotify, X and Bluesky join the type list; content links require at least one language at input; one shared add/edit modal serves the community page and the profile, warns when a URL edit will drop a pin, and offers the target's og:title as an editable label suggestion; empty country cards prompt their own NC to pin; and the help guides explain how members add links and how officials curate them. **Done when** the page renders that structure, the engine, backend and editor enforce the model rules, tests are green, and `wiki/architecture.md`, `wiki/access.md`, `wiki/hazards.md` and both help guides are updated. Plan in `board/community-page.md` — read that, not this line.
