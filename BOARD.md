...
- Say on every feedback issue filed from beta that it came from beta — beta and production file into the same repository through the same GitHub App, and today nothing in an issue tells a tester's report from a player's. **Done when** an issue filed from beta carries an environment line in its body and a `beta` label, one filed from production carries neither, and the feedback triage procedure names the `beta` label alongside the ones it already describes.
