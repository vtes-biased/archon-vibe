...
- Let an archival result carry a player count without a roster, and make an archival record correctable in-app — we cannot express "42 players, roster unknown" today, so a rounds-less import has no field size and the rating coefficient has nothing to work from. Measured 2026-08-16: **all 3627 pre-2014 tournaments are rounds-less**, so the backfill is the whole historic corpus and this is a hard prerequisite of the Hall of Fame's player-count floor, not a parallel track — introducing that floor first would evict essentially the entire historic Hall of Fame. **Done when** a rounds-less tournament can hold an attested player count, that count feeds the rating coefficient, and an official can correct an archival record without a script. Beware: `player_count` is already taken as a key by league scoring (`wiki/hazards.md`).
