...
- Stop an abandoned sync-stream connect from burning a production database connection: a client that hangs up part-way through a connect cancels a query still in flight, and the pool discards a connection it can no longer roll back — three times in 27 hours of production logs, against a pool of eight. The public API already shields its queries for exactly this reason; the app's own stream, which every client hits on every reconnect, does not. **Done when** the stream path's database reads survive a mid-query hang-up with the connection returned to the pool, a deliberate local hang-up produces no discarded connection, the trap is recorded in `wiki/hazards.md`, and `wiki/sync.md` no longer reads as though the resync early-return were the general protection when it spares only the resyncing client. Context in `board/sse-pool-churn.md`.
