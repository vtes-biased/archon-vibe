...
- Make the daily background jobs actually run — sanction expiry, the promo stock pass, the deleted-object purge and the archive sync are each registered on a 24-hour timer that the service's own daily restart kills before it can ever fire, so not one of them has run in any environment: expired sanctions never lift, soft-deleted rows are never reclaimed, and the historic Hall of Fame has no live source. The three that predate this release are already dead on production, and shipping 1.0.4 turns the archive sync from a working daily job into a fourth. **Done when** all four are observed running on beta across a restart cycle, the scheduled-jobs table in `wiki/architecture.md` matches what runs, and the restart-versus-timer trap is named in `wiki/hazards.md`. Context in `board/scheduled-jobs.md`.
...
- Make each deployment role's own defaults the single source of its parameter contract — the beta and production playbooks each spell the full parameter list out by hand, so a parameter added to one caller and not the other is silently skipped instead of failing: the operator scripts were wired on production only, and beta shipped without them until someone tried to rehearse a production run there. Every role parameterised the same way is exposed the same way, and the fix is defaults that make omission correct, not a check that catches it. **Done when** every parameter a role reads is documented in that role's defaults with a default that makes omission correct, each playbook passes only what genuinely differs between the two environments, and the deployment section of `wiki/dev.md` says where a role's contract lives.
