...
- Discord login drops the post-login redirect: `handleDiscordLogin` navigates to `/auth/discord/authorize` without passing the `?redirect=` param the consent page set, so a user sent to `/consent` by a third-party OAuth client (e.g. rulings.krcg.org) who logs in via Discord lands on `/` instead of resuming consent — passkey and email login honour it via `successTarget()`, and the `login_hint=discord` auto-path already passes it through. **Done when** the Discord button forwards the same-origin-guarded redirect like the other login methods, and a test covers `/login?redirect=<consent-url>` → Discord → back at the consent URL.
