...
- Give the data plane one place to migrate stored values. A stored document drifts from the code that decodes it, and the backend decodes strictly, so a changed shape or value breaks every read of the affected rows until they are rewritten — the schemaless table removes *schema* migrations, not these. The two that exist today disagree with each other: the role rename rewrites rows from `schema.sql` before the app serves, and so also fires from every ops script that merely opens a connection, while the link-moderation collapse leaves a script to run after the deploy, with every read it breaks failing in the gap. Replace both with ordered, self-guarding migrations run from the app's lifespan before it serves, projections recomputed by the one code path that owns them, and a lint that fails when a migration has no live section in post-deploy — so an entry dies in the same commit that retires its proof. **Done when** `schema.sql` carries no data statement, the collapse script is gone, both migrations run pre-serve, the lint fails on an unpaired entry, and `wiki/dogmas.md`, `wiki/architecture.md`, `wiki/post-deploy.md`, `wiki/dev.md` and `wiki/hazards.md` carry it. Context in `board/data-migrations.md`.
