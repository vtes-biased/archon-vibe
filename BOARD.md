...
- A browser tab whose update stream dies without notice keeps reporting itself online and serves stale data until someone reloads it — on production this swallowed a member creation and a role change over a 2h45m window, with no warning of any kind. Reconnection hangs entirely off the browser's connection-error event, which never fires when a socket dies silently, and the server's 30-second heartbeat is a stream comment the browser discards before any code can see it, so neither side can notice the silence; the status chip reads the operating system's network state, not the stream's. **Done when** a tab whose connection is dropped without notice recovers by itself within a heartbeat or two, the status chip stops claiming online while the stream is down, the recovery path goes through the existing backoff rather than around it, and `wiki/sync.md` records the heartbeat contract and the recovery rule beside the queue-overflow decision it extends. Context in `board/sse-silent-death.md`.
- Members created without a city are refused by the vekn.net registry and have never reached it — three of them have been retrying every hour and failing since 15 August, visible only as log noise, so the sponsor who created them believes they are registered. **Done when** the stuck members have a city (the country capital), a member created without a city pushes with a default or fails where the sponsor can see it, and `wiki/vekn.md` records what the registry requires on member creation. Context in `board/vekn-push-required-fields.md`.
