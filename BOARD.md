...
- Give the new-member invite email a lifetime measured in days rather than the 15 minutes it currently shares with signup and password-reset links, and give a recipient whose link has died a visible way back in — the expired-link page offers only "back to login" today, though a password reset on the same address already works for an invited member who never set one. **Done when** an invite link survives at least a day, the email states its real lifetime instead of a hardcoded fifteen minutes, the expired-link page routes the recipient to a fresh link, and the magic-link row in `wiki/access.md` states the per-purpose lifetimes and that recovery. Context in `board/invite-link-lifetime.md`.
- `syncManager.refresh()` clears IndexedDB while the old stream is still open — `connect()` is what disconnects, and it runs after the clear — so a live frame landing in any of the clear's await gaps writes a row at the *outgoing* access level into the freshly emptied store, and the snapshot refill that follows never removes it. Reachable on every level change that calls it: cross-tab logout, the profile save, the role and country changes. **Done when** the stream is closed before the stores are cleared, and `wiki/sync.md` states that ordering beside the resync it serves.
