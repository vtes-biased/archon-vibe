...
- Stop a stale local frontend build from failing the E2E suite: the test image's build context copies the working directory's `frontend/.svelte-kit` over the clean state it just built, so anyone who has run the dev server locally gets false failures — 22 of 26 today — and misattributes them to their own change. **Done when** `just test-e2e` passes from a working directory holding a stale `.svelte-kit`. No wiki page changes: the fix removes the trap rather than recording one.
