...
- Restore `POST /oauth/revoke` (RFC 7009, removed in `3abb708`) so a third party can hand back a user's `oauth_access`/`oauth_refresh` pair without the user going to their profile — the wiki deferred it on "the daemon grant lands", which it now has. A daemon token is stateless and out of scope: there is nothing to revoke but the client's `active` flag. **Done when** the endpoint revokes a live token pair, an unknown token still answers 200 per the RFC, and `wiki/access.md` lists it.
