...
- Make reopening a finished tournament destroy nothing: return it to **Playing** with the finals table, winner, finalist flags and deck stamps all intact, so the organizer lands back in the final they need to correct and re-runs **Finish Finals** when done — and let **Cancel Finals**, which already handles the open-rounds cap that reopen's blanket sweep gets wrong, be the one path that discards a final. Decklists still unpublish, since publication is derived from the finished state and returns with it; an event finished without a final still reopens to Check-in, having no final to return to. **Done when** reopening preserves the finals and its winner, `wiki/tournaments.md` states the new semantics and drops reopen from the deck-stamp release rule, the reopen confirmation copy matches in all five locales, and the organizer guide's step 13 and its wrong-scores FAQ no longer describe a discard.
