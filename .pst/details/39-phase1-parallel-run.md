...
- **D-1 push sequencing.** `vars.yml` hardcodes `VEKN_PUSH: "true"`. Either (a)
  deploy with push temporarily false, seed+merge+audit-zero, then flip true +
  redeploy (safest), or (b) trust the seed/merge/audit to finish inside the
  first `VEKN_PUSH_INTERVAL_HOURS: 1` window. **Recommend (a).**
- **D-2 bootstrap on a live box.** `bootstrap.yml`/`database.yml` run archon's
  common+postgresql roles on the box legacy already provisioned. Rehearse with
  `just dry-prod` and read the diff before any live run — confirm no clobber of
  legacy's nginx vhost, ufw rules, or certbot timers.
- **D-3 ansible_user.** `hosts.ini` connects as `ubuntu` (first bootstrap). #83
  deploy path assumes `ansible_user=lpanhaleux`. Confirm which user the deploy
  account is and that its key is on the box.
...
- [ ] **A-1 phase-1 domain override (committed).** Create the override that sets
      `domain_main=new.archon.vekn.net` (redirect URIs / `WEBAUTHN_RP_ID` /
      `site_url_base` all derive). `domain_bot` stays `bot.archon.vekn.net`
      (already prod-final). Everything else in `vars.yml` stays prod-final. Wire
      it into `deploy-prod` (group_vars layer or `-e @overrides`). **Does not
      exist yet.** Must be reverted/removed at Phase 2 flip (#40).
...
- [ ] **A-3 pin the release.** Choose the known-good CI tag to deploy (beta is on
      ≥ v0.1.6; ensure #185/#186 and any post-beta fixes are in it). Deploy uses
      `RELEASE_TAG=vX.Y.Z just deploy-prod`.
...
- [ ] **B-1 DNS A records** `new.archon.vekn.net` + `bot.archon.vekn.net` →
      `46.226.104.123` (both empty now). Certbot HTTP-01 hard-fails until they
      resolve. Same IP as legacy — no TTL games.
...
- [ ] **C-3 legacy merge seed (#115)** — `migrate_from_archon.py --merge` reading
      the live `archondb` over the socket (`OLD_DATABASE_URL=postgresql:///archondb?host=/var/run/postgresql`).
