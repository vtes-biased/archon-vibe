...
@pytest.mark.asyncio
async def test_non_member_cannot_set_community_links(test_client: AsyncClient, test_db):
...
@pytest.mark.asyncio
async def test_community_links_invalid_type_rejected(test_client: AsyncClient, test_db):
    """Invalid link type returns 422."""
    user = await _insert_user(roles=[Role.NC], vekn_id="1000002")
    response = await test_client.patch(
        "/auth/me",
        json={
            "community_links": [{"type": "myspace", "url": "https://myspace.com/test"}]
        },
        headers=make_auth_header(user.uid),
    )
...
async def test_community_links_bad_url_rejected(test_client: AsyncClient, test_db):
    ...
    response = await test_client.patch(
        "/auth/me",
        json={"community_links": [{"type": "discord", "url": "ftp://discord.gg/test"}]},
        headers=make_auth_header(user.uid),
    )
    assert response.status_code == 422
