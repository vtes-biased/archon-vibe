...
**Data export** — `GET /snapshot?download=1` serves the same content re-enveloped as a single-entry `.zip` attachment (`Content-Disposition: archon-export-<level>-<date>.zip`, holding the same-named `.jsonl`; no `Content-Encoding`, so the browser writes the archive to disk rather than inflating it). Zip over the stored `.gz` because Windows opens it natively; `_iter_snapshot_zip` inflates and re-deflates through `zipfile` into a non-seekable sink drained every 64 KB, so heap stays bounded and the archive streams (per-entry data descriptors). The Admin section of the profile page exposes it as a download button for IC (→ `full` level), behind a confirm modal warning that the file carries member PII; by hand it's `curl -OJ 'https://…/snapshot?download=1&token=<JWT>'`. It's an *export*, not a backup: up to 15 min stale, no soft-deleted tombstones, no `VtesCard` static data — `pg_dump` remains the backup tool.
...
**Self-echo suppression on go-online** — when a device brings *its own* tournament online, the resulting `offline_mode=false` broadcast would otherwise echo back over SSE and trip the lock-loss path before the go-online HTTP response clears local state. Two layers prevent this: (1) the server self-excludes the initiating device from the tournament broadcast — `broadcast_precomputed(bd, exclude_device_id=request.device_id)` skips connections whose `device_id` (sent as a `/stream` query param, the client's `getDeviceId()`) matches; the resolved-users/decks/sanctions/ratings frames still reach it. (2) While `goOnline()` is in flight the client adds the uid to `goingOnlineUids`, and `lostOfflineLock()` short-circuits to `false` for it — so the go-online **HTTP response is the sole authority** on the lock outcome during that window, ignoring all SSE lock-state frames (a concurrent force-unlock/takeover is then reported once, via the 410/409 path below, not twice).

**go-online server-managed field re-pull** — before saving the device's offline snapshot, the server re-stamps a fixed set of fields from the authoritative locked row: `banner_path`, `external_ids`, `checkin_code`, `vekn_pushed_at`, `vekn_results_stale`. These are never written by the WASM engine; they can be updated server-side during the offline window (VEKN sync writes `external_ids`/`vekn_pushed_at`; a re-uploaded banner writes `banner_path`; an SA/DQ issued by another organizer can flip `vekn_results_stale`). Trusting the device's stale values would revert those changes. "Server wins" for non-engine fields, same as `organizers_uids`.

**go-online atomicity** — the tournament snapshot, offline sanctions, and offline decks all persist inside the SAME locked transaction; broadcasts fire post-commit. A connection drop mid-push therefore leaves the server row still offline and the whole push retryable — never a committed online row with the offline objects silently lost. The response is `{tournament, summary}`: the recomputed authoritative row plus an outcome summary (players matched / accounts created / decks synced) the client surfaces as a toast.

**go-online 410/409 guard** — `POST .../go-online` returns **410 Gone** if the server is no longer in offline mode (already unlocked or brought online by another path), or **409 Conflict** if another device force-took-over the lock. The client catches both, clears orphaned local offline state, and re-raises the lock-lost warning ("unlocked or taken over" covers both). This prevents a stale device from blind-overwriting authoritative state with its snapshot, and stops a 409 from wedging the device offline; SSE/snapshot delivers the current state instead. Reclaiming after a 409 is a deliberate separate force-takeover, not a silent clobber from this path.
