...
**go-online server-managed field re-pull** — before saving the device's offline snapshot, the server re-stamps a fixed set of fields from the authoritative locked row: `banner_path`, `external_ids`, `checkin_code`, `vekn_pushed_at`. These are never written by the WASM engine; they can be updated server-side during the offline window (VEKN sync writes `external_ids`/`vekn_pushed_at`; a re-uploaded banner writes `banner_path`). Trusting the device's stale values would revert those changes. "Server wins" for non-engine fields, same as `organizers_uids`.
