...
**Data export** — `GET /snapshot?download=1` serves the same file as an opaque `.gz` attachment (`Content-Disposition: archon-export-<level>-<date>.jsonl.gz`, no `Content-Encoding`, so the browser writes the compressed bytes to disk rather than inflating them). The Admin section of the profile page exposes it as a download button for IC (→ `full` level); by hand it's `curl -OJ 'https://…/snapshot?download=1&token=<JWT>'`. It's an *export*, not a backup: up to 15 min stale, no soft-deleted tombstones, no `VtesCard` static data — `pg_dump` remains the backup tool.
