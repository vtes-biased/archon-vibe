...
**Raffle prize link**: `RaffleDraw`'s optional `prize_promo_uid` (organizer-picked from the same gated catalog, serde-only field on the draw's `json::object` literal) is display-only — never written to `promos_distributed`, so a raffled promo doesn't double-count. Both organizer and player raffle views resolve it to name + cached image; `PromosDistributedEditor.svelte` surfaces unreported raffled promos only as a dismissible pre-fill hint (`count_promo_references` also counts `raffles` prize references and `promo_ledger` rows for the hard-delete guard).

**Inventory ledger and computed stock**: `promo_ledger` (side table, not synced — SYNC.md's Non-Synced Side Tables / offline-first carve-out) is the append-mostly source of truth for holder inventory — `kind` `intake` (print batch received from BCP, credited to the receiving holder in `from_uid`, no `to_uid`), `assignment` (stock moves holder→holder), or `distribution` (a non-tournament exit, no `to_uid`); corrections are compensating rows (negative `qty`), never edits. A holder with no recorded intake is the old fallback: stock can still go negative from assignments/distributions alone (unrecorded origin, UI hides the negative source). `POST/GET /api/promos/ledger`: POST is self-sourced (`from_uid` defaults to the actor) except IC, who may record for another holder (unrecorded-origin sources need no prior stock — warn, never block); `intake` is additionally officials-only — NC may intake into their own pool, IC into any holder's, plain members cannot intake at all. `assignment` rejects `from_uid == to_uid` (400 — credits and debits the same holder, a no-op in the recompute; use `intake` instead). GET returns the whole role-scoped ledger with no pagination (officials = IC or NC see every row, everyone else only rows they're party to). `promo_stock.py`'s `recompute_promo_stock()` re-derives, per affected promo, the full per-holder `Promo.holdings` from every ledger row plus live tournaments' `promos_distributed` attributed to each report's `promo_stock_source_uid` (`get_tournament_promo_attributions`), and merges the matching keys into each holder's `User.promo_stock` (dropping stale keys for holders who fall out, e.g. a changed stock source) — both full-projection fields, so remaining stock always streams through the normal SSE pipeline instead of being derived client-side. Triggers: every ledger POST, a catalog PUT (self-heal against a concurrent recompute overlap), any tournament save whose `promos_distributed` set changes (`_promo_recompute_diff` diffs old vs. new, recomputes the union — `ReportPromos`, offline push/go-online, soft-delete), `users/merge` (remaps ledger holder uids first, then a full recompute), and a daily self-healing pass (`run_promo_stock_recompute`, scheduled in `main.py`). Fire-and-forget (`schedule_recompute` → `asyncio.create_task`) so route handlers never block on it. Hard invariant: recompute only reads/attributes `Tournament.promos_distributed`, never writes it — the offline device stays sole authority over that field. `PromosDistributedEditor.svelte` warns — never blocks — when a submitted report drives the submitter's own stock negative (checked only when the submitter is also the report's stock source).

**Community tab UI**: the `/users` page's "Promos" tab (`PromosTab.svelte`, a sibling of Community/Members — not nested inside `CommunityTab.svelte`) is the single hub, role-gated progressive disclosure like Community-tab moderation. `PromoGallery.svelte` — public, works logged-out and offline — shows active promos as image cards (kind-icon fallback tile when no image) with rank/league gating badges and an officials-only "show retired" toggle. A signed-in holder with nonzero `User.promo_stock` additionally sees `OwnStockCard.svelte` (view-only remaining counts). IC/NC additionally see `PromoInventoryPanel.svelte`: per-promo `Promo.holdings` rows (offline-readable; holder names resolved defensively, falling back to the uid for soft-deleted/missing users) plus online-only ledger controls — `RecordMovementModal.svelte` (intake/assignment/distribution entry; the assignment target and, for IC, the source/receiving holder are picked via `UserPicker.svelte`, a single-select IndexedDB-search typeahead), a collapsible `PromoLedgerList.svelte` (fetches the whole role-scoped ledger on open; the carve-out's client-side filter/aggregate step), and a client-generated CSV export (`promo-csv.ts`, a dependency-free RFC-4180 builder that re-fetches the ledger and resolves every holder uid to a name before download). IC alone gets `PromoEditModal.svelte` for mint/edit/retire/delete plus image upload (plain file input, no cropper — promo art is pre-made variable-aspect scans).
...
- `sanctions.rs` — single source for the Judges-Guide v2 penalty reference (categories/subcategories, English labels, baselines, escalation ladder; `sanction_reference_json`). `backend/src/models.py` derives `SUBCATEGORIES_BY_CATEGORY`/`BASELINE_PENALTIES` from it at import; frontend reads it via `getSanctionReference()` (`engine.ts`); the Discord bot fetches it via the public `GET /sanctions/reference`. Distinct from `tournament/sanctions.rs` (SA effective-round resolution, below).
...
- `tournament/standings.rs` — `compute_preliminary_standings` (GW/VP/TP/toss sort). GW and TP are **recomputed** per table from raw VPs + current sanctions (`sanctions::table_sa_adjustments` → `compute_gw`/`compute_tp`), so an SA issued *after* a round was scored re-decides GW and re-ranks/re-averages TP — the frozen seat `result.gw`/`result.tp` would otherwise go stale. VP sums raw per-seat VP then subtracts the full SA penalty (`-1` per SA via `sa_vp_penalty`), which may go negative; per-seat `result.vp` stays raw for display. Both `table_sa_adjustments` and `sa_vp_penalty` consume `sanctions::resolve_sa_effective_rounds` (same file) so GW/TP and VP always agree on which round each SA targets. DQ'd players (state `Disqualified` or active DQ sanction) have their VP/GW/TP zeroed and are sorted last; opponents keep all earned scores. Non-competing (proxy) players (`non_competing == true`) are also sorted last with rank "—" but their VP/GW/TP are kept (not zeroed). `compute_rating_vp_gw` (single source for the backend rating/VEKN-push paths) applies the same rule and additionally includes finals VP/GW; prelim comes from the rounds when present, else from the (prelim-only) standings row for round-less VEKN imports; when no `finals` object recorded the win it credits the tournament winner a +1 GW (a no-final import — native no-final leaves `winner==""`, so it stays inert there); returns `(0, 0)` early for DQ'd or non-competing players. `compute_final_standings`: winner = rank 1, other finalists share rank 2 (VEKN §3.7.5), non-finalists competition-ranked from finalist_count+1; whether a final happened is read from the per-player `finalist` flag, not from finals seating data.
- `deck.rs` — deck parse/validate, TWDA export.
- `ratings.rs` — rating points computation; skips DQ'd players entirely (no RTP entry); `_player_count` stays inclusive of DQ'd players (tournament-rules A.2). `ranking_eligibility` is the single-sourced rules-3.1/3.1.6 gate (≥8 players who played AND a played final; open-rounds/self-organized never) consumed by both `ratings.py`'s inclusion filter and the frontend ranked/unranked badge + RtP column.
...
- **Per-round lifecycle** (backend, not Rust engine): every round/finals lifecycle event (StartRound, SelfOrganizeRound, StartFinals, RestoreRound, FinishRound, CancelRound, FinishFinals, FinishTournament, CancelFinals) resets the timer to a fresh full **paused** `TimerState()` and clears `table_extra_time`. Starting a round NEVER launches the clock — players need time to get seated, so the organizer starts it explicitly via `/timer/start`. `AddTable` is excluded so a mid-round table add never resets a running clock. The single global timer is meaningless with parallel rounds (each self-organized pod is its own round), so the frontend and bot deactivate it whenever more than one round is live.
...
- **#judges privacy** — private from creation (`@everyone` denied VIEW_CHANNEL+CONNECT). Every reconcile also syncs its membership to the `/setup` runner plus the tournament's `organizers_uids`, so a web-app organizer add/remove propagates automatically; falls back to the backend `User.discord_id` (delivered via the scoped stream's participant frames) for organizers who never ran `/register`, with a one-time `#judges` notice for anyone still unresolved. `structure_signature` keys on the organizer set directly so this reconciles in every state, not just `Playing`; the same sync retrofits pre-privacy channels found without the deny override.
...
**Channel permissions**: #announcement — @everyone DENY SEND_MESSAGES, bot allowed; table voice — @everyone DENY CONNECT, per-player + organizers ALLOW CONNECT+SPEAK (organizers may join any table to judge); #judges — @everyone DENY VIEW_CHANNEL+CONNECT, bot + organizers ALLOW VIEW_CHANNEL+CONNECT; synced idempotently.
...
- Anonymous (no-token) feeds render venue/address into LOCATION — both are public-projection fields (SYNC.md), consistent with the `.ics` being an advertising artifact mirroring vekn.net's public event calendar; venue granularity is the organizer's data-entry choice.
...
`twda.py` opens or updates a GitHub PR against the [TWDA repo](https://github.com/GiottoVerducci/TWD) — idempotent (branch `archon/{vekn_event_id}` + file `decks/{id}.txt`, create-or-update). `maybe_submit_twda()` (`routes/tournaments.py`) gates and fires it from five triggers: the finish action; a winner-deck upsert on an already-finished tournament (by an organizer, or by the winner adding their *first* deck — post-finish, players can add but not replace, so late uploads and post-event edits reach the archive); the manual `POST /{uid}/push-vekn` organizer publish; `batch_push()` after a successful results push (`vekn_push.py`, covers events finished offline or whose VEKN event id only arrived via the same batch); and `go-online` for a tournament finished offline (usually records an immediate `no_vekn_event` skip since the event isn't pushed yet — `batch_push` retries once it is). Files: `backend/src/twda.py`, `engine/src/deck.rs` (`export_twda`).

Every attempt records its outcome on `Tournament.twda_status` — `submitted` (with PR URL), `skipped` (with a reason code: no winner, Limited format (draft/sealed decks aren't archived), <10 players played, unranked per the engine's `ranking_eligibility` (the same predicate as the ranked badge — `rank` is the Basic/NC/CC championship axis and never gates TWDA), no VEKN event id, GitHub App unconfigured, or winner has no deck), or `failed` — organizer/full projection only (`_TOURNAMENT_MEMBER_EXCLUDE` in `access_levels.py`), shown as a status line on the finished-tournament organizer view (`FinishedResults.svelte`). Replaces the prior silent fire-and-forget.
...
| Promo stock recompute | Daily | `promo_stock.py` | Self-healing full recompute of `Promo.holdings`/`User.promo_stock` from the ledger + tournament attributions |
