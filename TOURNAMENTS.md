...
**`max_players`** (int, 0 = none) is a **soft registration cap**: purely advisory for venue capacity. The UI warns the actor on self-register and organizer AddPlayer past the cap and shows N/cap in the players list — the engine never blocks on it, and there is no waitlist (auto-promotion deliberately deferred).
...
- **Round targeting** (per §1.1.3): the −1 VP lands on the player's **current game if one is in progress, otherwise their most recently played game — never a future round.** Concretely, the effective round is the highest round index in which the player is seated, **including the finals**, which participates as round index `len(rounds)` (the same sentinel `SetScore` uses; the backend accepts it once a finals table exists). The SA's stored `round_number` is the *fixed issue-time record* of the game the judge ruled on; the engine honors it when the player was seated in that round and otherwise **redirects** to the player's most-recently-seated round (so an SA referencing a round the player sat out lands on a game they actually played, not nowhere). A player who has not yet played any round has no game to penalize, so the SA contributes nothing until they do. (Issuing an SA *before round 1 pairings* is not supported — the backend requires an existing round.)
- **Finals-round SA**: penalizes the **finals result**, not the preliminary totals — prelim standings exclude finals-round SAs from their VP penalty; the −1 VP instead applies to the finals GW derivation (it can flip who wins the tournament) and to the rating VP total (which includes finals VP). Because ratings and placement read the *stored* finals seat `result.gw`/`result.tp` and `tournament.winner`, every standings recompute re-scores a `Finished` finals table from raw VPs + current sanctions and re-derives the winner (when one is set) via the same `compute_gw_finals` call `SetScore` and `FinishFinals` use, so a finals SA issued or lifted *after* the finals were scored still lands. If `CancelFinals` later nulls the finals, a finals-targeted SA gracefully redirects to the player's last prelim round (the most-recent-seated rule) — it is not lost — and re-lands on the finals when a new final seats the player.
...
- **Offline sanction management**: event-level sanctions of a device-locked tournament are created/deleted offline — written to IndexedDB (tagged in `offline_sanctions:` metadata), with the client mirroring the server side effects: DQ player-state flips (incl. the playable restore) and a standings recompute via the WASM `updateStandings` (same shared Rust as `PyEngine.update_standings`, fed from IDB sanctions). At go-online the sanctions are pushed with the snapshot; the server asserts DQ states in the locked transaction and runs ONE authoritative recompute after saving them. VEKN-wide suspension/probation stay online-only.
