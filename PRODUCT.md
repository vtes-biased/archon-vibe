...
**VEKN Judge Certifications** (profile titles, NOT elevated tournament permissions): **Judge** (certified VEKN judge), **Sheriff** (junior — stored as the value `Judgekin`), **Rulemonger** (rules specialist). They appear on member profiles but grant no extra power in tournament management.
...
**Needs**: sponsor new VEKN members; appoint Princes in their own country (NC) or anyone (IC); view all tournaments/results in jurisdiction; access player contact info (NC: same country; IC: all); view + manage sanctions.
...
- **Members**: profiles (name, country, city, socials, contact), avatar upload (client crop + server compress), VEKN ID claim/sponsor/link/abandon/force-abandon, roles (IC, NC, Prince, Judge, Sheriff, Rulemonger, Ethics, PTC, Playtester, DEV), IC-only account merge, cooptation tracking, privacy-filtered directory. Who may do what: the Authorization matrix in ARCHITECTURE.md.
- **Tournament core**: full config (format/rank/proxies/multideck/online/venue/dates/tz/country/max_rounds, `max_players` warn-only soft registration cap), state machine Planned→Registration→Waiting→Playing→Finished (+reopen), registration + check-in (single/all/reset), simulated-annealing seating (9 priorities) incl. staggered seatings for 6/7/11, VP entry with oust-order validation, GW/TP auto-compute, judge override/unoverride, standings (GW>VP>TP), finals qualification + manual/random toss, finals AlterSeating, round/registration/tournament reopen + cancel, delete (any state, until pushed to VEKN). **Open Rounds** (`max_rounds > 0`): per-player cap — each player plays up to `max_rounds` rounds of a continuously-run pool; the tournament MAY run more total rounds than `max_rounds`; players hit their cap individually and move to state `Completed` (finals-eligible). Non-VEKN format gaining traction online. `max_rounds = 0` = no cap.
...
- **Raffle**: random draw from pools (AllPlayers/NonFinalists/GameWinners/NoGameWin/NoVictoryPoint), draw/undo/clear; optional prize from the promo catalog (same gating as distribution reporting), display-only — never auto-added to the distribution report.
- **Promos**: IC-managed catalog (BCP promo cards/packs, alt-art or unreleased, no krcg link) with optional rank/league distribution gating, surfaced to everyone (incl. logged-out/offline) as a public gallery on the Community "Promos" tab, with gating badges and retired items hidden by default; organizer per-event distribution reporting (`ReportPromos`, replace-whole-list, no state gate — post-finish corrections are first-class), with a post-finish entry nudge. Inventory ledger (intake/assignment/distribution movements — intake, a print batch received from BCP, is officials-only: NC their own pool, IC any holder; assignment rejects a self-transfer as a no-op, use intake instead; officials-only REST read, no pagination, client-generated CSV export) is the source of truth for server-computed remaining stock — shown as an IC/NC inventory panel (`Promo.holdings`) and a holder's own-stock card (`User.promo_stock`), never hand-entered or client-derived; the distribution editor warns — never blocks — when a report drives the submitter's own stock negative.
...
- **Social & discovery**: shareable finished-tournament PNG + text (localized, on both organizer and player finished views); per-tournament and per-league og:image crawler stubs; iCal feed (personal via calendar_token / country / global, online toggle; token stripped from SSE; personal feeds keep recently-finished own events ~90 days); per-event single-VEVENT `.ics` download; agenda matching; tournament list filters (country/format/search, My Agenda, include-online).
