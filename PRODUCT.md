...
- **Promos**: IC-managed catalog (BCP promo cards/packs, alt-art or unreleased, no krcg link) with optional rank/league distribution gating; organizer per-event distribution reporting (`ReportPromos`, replace-whole-list, no state gate — post-finish corrections are first-class), with a post-finish entry nudge.
