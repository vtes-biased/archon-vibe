...
organizer wherever they may replace the deck, and written to the TWDA entry — both
stripped of deckbuilder import noise ([vekn](vekn.md#outbound)); a
...
country, global), agenda matching with the member's own hide and add overrides,
list filters including National/Continental championship rank.
...
tournament list. Shape and the decisions behind it: [design](design.md#landing).
...
**The cap never refuses a registration.** Past the venue cap a sign-up lands on a
