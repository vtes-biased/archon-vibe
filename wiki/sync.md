...
|---|---|---|---|---|
| user | NC/Prince with contact + community links; IC without contact; any other user with non-empty community links as a minimal no-name row (country, roles, links) | all users — no contact except an NC's or Prince's published one, no `deceased_by_uid`/`anonymized_by_uid`, no `github_login`/`github_id`; `deceased_at` and `anonymized_at` included; anyone with non-empty community links gets those included | everything except the owner-only `calendar_token`, `agenda_hidden` and `agenda_added` ([architecture](architecture.md#data-model)) | only users holding a `vekn_id`: uid, the id, country, roles, the four `CategoryRating` fields, `wins`, community links with their moderation value — no name, nickname, contact, city or avatar |
...
| sanction | none | full data | full data | none, permanently — the member lookup answers an app token the standing suspensions and probations off `full` ([public-api](public-api.md)) |
...
Payload `{tournament_uid, table, table_label, table_number, player_name}`:
`table` is the index, `table_label` null when no room covers the table, and each
reader then renders its own localized "Table N" from `table_number`, the Discord
...
resolves the venue-typed address through every place a member's address is held
([access](access.md#the-email-of-record)). It reconciles by
